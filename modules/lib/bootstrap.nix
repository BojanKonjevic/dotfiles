    ...
    apps.bootstrap = {
      ...
      program = builtins.toString (
        pkgs.writeShellScript "bootstrap" ''
          ...
          ok "username   → $DETECTED_USER"
          ok "home       → $DETECTED_HOME"
          ...
          # ── Interactive prompts ───────────────────────────────────────────
          header "A few things I can't detect automatically…"
          ...
          prompt "Weather city (wttr.in format)"    "London"                  WEATHERCITY
          ...
          WEATHERCITY_ESCAPED="''${WEATHERCITY// /+}"
          ...
          rec {
            ...
            # ── Weather ────────────────────────────────────────────────────
            weatherCity    = "$WEATHERCITY_ESCAPED";
          }
          USERNIX
          ...
          if [[ "$HOSTDIR" == "desktop" ]]; then
            ...
            # Copy default.nix from desktop as a template — hardware.nix
            # will be overwritten below so we just need the structure
          ...
          ok "Hardware config copied to modules/hosts/$HOSTDIR/hardware.nix."
