...
if systemctl is-active --quiet nix-daemon; then
  badge_ok "nix daemon"; echo
else
  badge_err "nix daemon"; echo
...
if ping -c1 -W2 cache.nixos.org &>/dev/null; then
  badge_ok "cache.nixos.org"; echo
else
  badge_err "no internet"; echo
  exit 1
fi
...
if $EVAL_OK; then
  badge_ok "flake eval clean"; elapsed $EVAL_TIME; echo
else
  badge_err "eval errors"; echo
  cat "$LOG"
  exit 1
fi
...
if $UPDATE; then
  ...
  wait $UPD_PID
  badge_ok "inputs updated"; elapsed $(( SECONDS - T0 )); echo
fi
...
OS_TIME=$(( SECONDS - T0 ))
if $OS_OK; then
  badge_ok "nixos switch"; elapsed $OS_TIME; echo
else
  badge_err "nixos switch"; echo
  exit 1
fi
...
HM_TIME=$(( SECONDS - T0 ))
if $HM_OK; then
  badge_ok "hm switch"; elapsed $HM_TIME; echo
else
  badge_err "hm switch"; echo
  exit 1
fi
...
if ! $SKIP_CACHIX && command -v cachix &>/dev/null; then
  ...
  (
    ...
    cachix push "$CACHIX_CACHE" "$HOME/.local/state/nix/profiles/home-manager"
  ) >"$LOG" 2>&1 &
  CACHE_PID=$!
  ...
  if $CACHE_OK; then
    badge_ok "cachix push"; elapsed $CACHE_TIME; echo
  else
    badge_err "cachix push"; echo
    cat "$LOG"
  fi
fi
